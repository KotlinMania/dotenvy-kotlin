# Claude Code Project Instructions — dotenvy-kotlin

## Project Overview

This is **dotenvy-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`dotenvy`](https://crates.io/crates/dotenvy).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/dotenvy/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/dotenvy>
