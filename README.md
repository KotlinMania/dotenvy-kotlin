...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`dotenvy`](https://crates.io/crates/dotenvy).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `dotenvy` crate will be cloned into `tmp/` (gitignored) when porting begins.
...
Original work copyrighted by the upstream `dotenvy` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
